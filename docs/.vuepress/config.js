...
export default defineUserConfig({
  ...
  bundler: viteBundler({
    viteOptions: {
      resolve: {
        alias: {
          // 把 content.css 替换成空文件
          'vuepress-theme-plume/lib/client/styles/content.css': path.resolve(__dirname, './empty.css'),
        },
      },
    },
  }),
  //需要判断是开发环境还是生产环境
  //   base: process.env.NODE_ENV === 'production' ? '/guns-library-describe/' : '/',
  base: '/',
  lang: 'zh-CN',
  title: '杭州智诚惠通科技有限公司',
  description: '智诚惠通前端文档示例中心',
  ...
  theme: plumeTheme({
    ...
    plugins: {
      ...
      shiki: {
        ...
        languages: ['shell', 'bash', 'typescript', 'javascript','css','vue','html'],
